...
describe("Login", () => {
  ...
  it("login disabled if missing GOOGLE_CLIENT_ID", () => {
    cySetup(cy);
    cy.visit("/");
    cy.get('[data-cy=login-menu]').find('[data-cy=login-button]').should("be.disabled");
  });
  ...
  it("login enabled if GOOGLE_CLIENT_ID is set", () => {
    ...
    cy.visit("/");
    cy.get('[data-cy=login-menu]').find('[data-cy=login-button]').should("not.be.disabled");
  });
  ...
  it("redirects to home page after logging out", () => {
    ...
    cy.location("pathname").should("contain", "lessons");
    cy.get('[data-cy=lessons]').children().should("have.length", 2);
    cy.get('[data-cy=login-option]').find('[data-cy=login-button]').trigger('mouseover').click();
