...
describe("edit lesson screen", () => {
  ...
  describe("new lesson", () => {
    it("new lesson has default values", () => {
      ...
      cy.get("input").should("have.value", "Introduction to the lesson,  e.g. 'This is a lesson about RGB colors'")
    });
      cy.get('[data-cy=question]').within($input => {
      cy.get("textarea").should("have.value", "Question the student needs to answer, e.g. 'What are the colors in RGB?'")
    });
      cy.get('[data-cy=image]').should("have.value", "");
      cy.get('[data-cy=expectations]').children().should("have.length", 1);
      cy.get('[data-cy=expectation-0] [data-cy=edit-expectation]').within($input => {
      cy.get("input").should("have.value", "Add a short ideal answer for an expectation, e.g. 'Red'")
    });
      cy.get('[data-cy=expectation-0]').get('[data-cy=hints]').children().should("have.length", 1);
      cy.get('[data-cy=hint-0] [data-cy=edit-hint]').within($input => {
      cy.get("textarea").should("have.value", "Add a hint to help for the expectation, e.g. 'One of them starts with R'")
    });
      cy.get('[data-cy=conclusions]').children().should("have.length", 1);
      cy.get('[data-cy=conclusion-0]').get('[data-cy=edit-conclusion]').within($input => {
      cy.get("textarea").should("have.value", "Add a conclusion statement, e.g. 'In summary,  RGB colors are red, green, and blue'")
    });
    });
    ...
    it("edits a new lesson", () => {
      ...
      cyMockDefault(cy)
      cy.visit("/lessons/edit");
      cy.get('[data-cy=lesson-name]').fill("Review Diode Current Flow");
      cy.get('[data-cy=lesson-id]').fill("review-diode-current-flow");
      cy.get('[data-cy=intro]').fill(
        "This is a warm up question on the behavior of P-N junction diodes."
      );
      cy.get('[data-cy=question]').fill(
        "With a DC input source, does current flow in the same or the opposite direction of the diode arrow?"
      );
      cy.get('[data-cy=image]').fill(
        "https://cdn.jpegmini.com/user/images/slider_puffin_before_mobile.jpg"
      );
      cy.get('[data-cy=expectation-0] [data-cy=edit-expectation]').fill(
        "Current flows in the same direction as the arrow."
      );
      cy.get('[data-cy=expectation-0]').get('[data-cy=hint-0]').get('[data-cy=edit-hint]').fill(
        "What is the current direction through the diode when the input signal is DC input?"
      );
      cy.get('[data-cy=conclusion-0]').get('[data-cy=edit-conclusion]').fill(
      ...
      cy.get('[data-cy=intro]').within($input => {
      cy.get("input").should("have.value", "This is a warm up question on the behavior of P-N junction diodes.")
    });
      cy.get('[data-cy=question]').within($input => {
      cy.get("textarea").should("have.value", "With a DC input source, does current flow in the same or the opposite direction of the diode arrow?")
    });
      cy.get('[data-cy=image]').within($input => {
      cy.get("input").should("have.value", "https://cdn.jpegmini.com/user/images/slider_puffin_before_mobile.jpg")
    });
      cy.get('[data-cy=expectation-0]').get('[data-cy=edit-expectation]').within($input => {
      cy.get("input").should("have.value", "Current flows in the same direction as the arrow.")
    });
      cy.get('[data-cy=expectation-0]').get('[data-cy=hint-0]').get('[data-cy=edit-hint]').within($input => {
      cy.get("textarea").should("have.value", "What is the current direction through the diode when the input signal is DC input?")
    });
      cy.get('[data-cy=conclusion-0 #edit-conclusion]').within($input => {
      cy.get("input").should("have.value", "Summing up, this diode is forward biased. Positive current flows in the same direction of the arrow, from anode to cathode.")
    });
    });
  ...
  describe("copy lesson", () => {
    it("loads a copy of the lesson", () => {
      ...
      cy.get('[data-cy=intro]').within($input => {
      cy.get("input").should("have.value", "introduction")
    });
      cy.get('[data-cy=question]').within($input => {
      ...
      cy.get('[data-cy=expectation-0]').get('[data-cy=hints]').children().should("have.length", 1);
      cy.get('[data-cy=hint-0]').get('[data-cy=edit-hint]').within($input => {
      cy.get("textarea").should("have.value", "hint 1.1")
    });
      cy.get('[data-cy=conclusions]').children().should("have.length", 1);
      cy.get('[data-cy=conclusion-0 #edit-conclusion]').within($input => {
      cy.get("input").should("have.value", "conclusion")
    });
    });
  ...
  it("can expand and collapse an expectation", () => {
    ...
    cy.get('[data-cy=expectation-0]').get('[data-cy=edit-expectation]');
    cy.get('[data-cy=expectation-0]').get('[data-cy=hints]');
    cy.get('[data-cy=hint-0]').get('[data-cy=edit-hint]');
    // collapsing an expectation hides hints
    cy.get('[data-cy=expectation-0]').get('[data-cy=expand]').trigger('mouseover').click();
    cy.get('[data-cy=expectation-0]').get('[data-cy=edit-expectation]');
    cy.get('[data-cy=expectation-0]').get('[data-cy=hints]').should("not.exist");
  ...
  it("adds and deletes an expectation", () => {
    ...
    cy.get('[data-cy=expectations]').children().should("have.length", 1);
    cy.get('[data-cy=expectation-0]').find('[data-cy=delete]').should("not.exist");
    // add and delete
    cy.get('[data-cy=add-expectation]').trigger('mouseover').click();
    cy.get('[data-cy=expectations]').children().should("have.length", 2);
    cy.get('[data-cy=expectation-0]').find('[data-cy=delete]').trigger('mouseover').click();
    cy.get('[data-cy=expectations]').children().should("have.length", 1);
  });
  ...
  it("adds and deletes a hint", () => {
    ...
    cy.get('[data-cy=hint-0 #delete]').should("not.exist");
    // add and delete
    cy.get('[data-cy=add-hint]').trigger('mouseover').click();
    cy.get('[data-cy=hints]').children().should("have.length", 2);
    cy.get('[data-cy=hint-0 #delete]').trigger('mouseover').click();
    cy.get('[data-cy=hints]').children().should("have.length", 1);
  });
  ...
  it("adds and deletes a conclusion", () => {
    ...
    cy.get('[data-cy=conclusions]').children().should("have.length", 1);
    cy.get('[data-cy=conclusion-0 #delete]').should("not.exist");
    // add and delete
    cy.get('[data-cy=add-conclusion]').trigger('mouseover').click();
    cy.get('[data-cy=conclusions]').children().should("have.length", 2);
    cy.get('[data-cy=conclusion-0 #delete]').trigger('mouseover').click();
    cy.get('[data-cy=conclusions]').children().should("have.length", 1);
  });
